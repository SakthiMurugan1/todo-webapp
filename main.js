...
addTaskField.value = '';
addTaskField.focus()
addAlreadyStoredTasks();

//Listeners
addTaskButton.addEventListener('click', addTaskToList); 
addTaskField.addEventListener('submit', addTaskToList);
...
function addTaskToList() {
    ...
    //Add fieldvalue into sessionStorage using MD5 hash a key
    let hash = MD5(fieldValue);
    sessionStorage.setItem(hash, fieldValue.toString())
...
function createNewListItem (fieldValue) {
    ...
    /*MD5 hash values of each taskItem is also stored in the delete button as an attribute.
    This helps in deleting the task from webstorage later*/
    let hash = MD5(fieldValue);
    deleteButton.setAttribute('taskHashId', hash);
...
function deleteTaskFromList (target) {
    ...
    /*Each delete button is attributed with the task has.
    This is now used to identify and delete from webstorage*/
    let taskHashId = target.getAttribute('taskHashId');
    sessionStorage.removeItem(taskHashId);
...
function addAlreadyStoredTasks () {
    let storedItem;
    for ( let i = 0, len = sessionStorage.length; i < len; ++i ) {
       storedItem = createNewListItem(sessionStorage.getItem(sessionStorage.key(i)));
       taskList.appendChild(storedItem);
    }
}